...
class CommandLine(object) :
	...
	def __init__(self, inOpts=None) :
		...
		'''
		import argparse
		self.parser = argparse.ArgumentParser(description = 'clusterMESA.py - Apply various clusrering methods to PSI data.',
...
def makeUMAP(dm,groups,colorD,markerD):
	...
	for num,s in enumerate(components):
		...
		for k,v in groups.items():
			...
			subplots[num].scatter(embedding[indices,0],embedding[indices,1],
				label=label[0],
				color=colorD[k[0]], 
				marker=markerD[k[1]], 
				alpha=0.2)
	...
	for num1,perp in enumerate(perplexities):
		...
		for num2,c in enumerate(components):
			...
			for k,v in groups.items():
				v = np.array(v)
				indices,label = np.array(v[:,0],dtype=int), v[:,1]
				subplots[num1][num2].scatter(Y[indices,0],Y[indices,1],
					label=label[0],
					color=colorD[k[0]], 
					marker=markerD[k[1]], 
					alpha=0.2)
