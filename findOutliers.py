...
class CommandLine(object) :
	...
	def __init__(self, inOpts=None) :
		...
		'''
		import argparse
		self.parser = argparse.ArgumentParser(description = 'findOutliers.py - Mission: Seek and destroy. Target: Outliers.',
		...
		# Add args
		self.parser.add_argument('--psiMESA', type=str, action = 'store', required=True, 
		...
		self.parser.add_argument('--nullMan', action = 'store', required=False, default=None,
								help='List of samples you want to use to use as baseline/background. Samples must be in table.')
		self.parser.add_argument('--outlierCutoff', type=int, action = 'store', required=False, default=3,
								help='Report events where zScore >= N (default 3).')
		self.parser.add_argument('--dpsiThrsh', type=float, action = 'store', required=False, default=0.1,
...
def main():
	...
	if len(nullSamps)<10:
		print("Less than 10 samples detected in null group. Too few. Exit.",file=sys.stderr)
		sys.exit(1)
	#load psi
	data = loadNPZ(pmesa)
	...
	for ePos,i in enumerate(matrix):
		...
		[print(mEvents[ePos],mSamps[outlierIndices[pos]],x,vals[pos],mean,std,sep="\t") for pos,x in enumerate(zScores) if x>=zthresh and abs(x-mean)>=pthresh]
